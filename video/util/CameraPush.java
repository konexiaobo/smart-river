...
public class CameraPush {
	...
	* @Date        :   2020/6/17
	* @Time        :   20:18
	* @Decription :   根据rtsp的地址和配置文件的地址建立拉流推流TCP连接
	*/
	public CameraPush from() throws Exception {
		...
		try {
			rtspSocket.connect(new InetSocketAddress(cameraPojo.getIp(),cameraPojo.getPort()), 1000);
		} catch (IOException e) {
			grabber.stop();
			grabber.close();
			rtspSocket.close();
			logger.error("与拉流IP：   " + cameraPojo.getIp() + "   端口："+cameraPojo.getPort()+"    建立TCP连接失败！");
			return null;
		}
		try {
	...
	*/
	public CameraPush to() throws Exception {
		...
		record.setVideoOption("crf", "2");// 画面质量参数，0~51；18~28是一个合理范围
		record.setGopSize(2);
